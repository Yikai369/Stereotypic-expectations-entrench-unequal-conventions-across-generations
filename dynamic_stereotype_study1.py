...
for run in range(num_run):
    for population_size in population_sizes:
        ...
        if population_size in (300, 600):
            ...
            wood_group_distribution = [int(val) for val in [n_agents_per_group*prop, n_agents_per_group*prop_reverse, n_agents_per_group*(1-prop-prop_reverse)]]
            stone_group_distribution = [int(val) for val in [n_agents_per_group*prop_reverse, n_agents_per_group*prop, n_agents_per_group*(1-prop-prop_reverse)]]
            house_group_distribution = [int(val) for val in [n_agents_per_group*0.1, n_agents_per_group*0.1, n_agents_per_group*0.8]]
        ...
        all_individual_attributes = [[i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12, i13]
                                     ...
                                     for i10 in binary for i11 in binary for i12 in binary
                                     for i13 in binary]
        ...
        original_agents_attributes = [list(tuple(i)+(i14, i15, i16)) for i in original_agents_attributes]
        unseen_attributes = [list(tuple(i)+(i14, i15, i16)) for i in unseen_attributes]
        ...
        for count1, agent_type in enumerate([0,1,2]):
            for count2, agent_subtype in enumerate([0,1,2]):
                ...
                if agent_type == 0:
                    group_id = [1,0,0]
                    if agent_subtype == 0:
                        list_subtype_agent = [Agent(0,0,original_agents_attributes[count+i], .95, .15, .05)
                                              for i in range(agent_distribution['wood'][0])]
                        count += agent_distribution['wood'][0]
                    elif agent_subtype == 1:
                        list_subtype_agent = [Agent(1,1,original_agents_attributes[count+i], .15, .95, .05)
                                              for i in range(agent_distribution['wood'][1])]
                        count += agent_distribution['wood'][1]
                    elif agent_subtype == 2:
                        list_subtype_agent = [Agent(2,2,original_agents_attributes[count+i], .1, .1, .95)
                    ...
                    group_id = [0,1,0]
                    if agent_subtype == 0:
                        list_subtype_agent = [Agent(3,3,original_agents_attributes[count+i], .95, .15, .05)
                                              for i in range(agent_distribution['stone'][0])]
                        count += agent_distribution['stone'][0]
                    elif agent_subtype == 1:
                        list_subtype_agent = [Agent(4,4,original_agents_attributes[count+i], .15, .95, .05)
                                              for i in range(agent_distribution['stone'][1])]
                        count += agent_distribution['stone'][1]
                    elif agent_subtype == 2:
                        list_subtype_agent = [Agent(5,5,original_agents_attributes[count+i], .1, .1, .95)
                    ...
                    group_id = [0,0,1]
                    if agent_subtype == 0:
                        list_subtype_agent = [Agent(6,6,original_agents_attributes[count+i], .95, .15, .05)
                                              for i in range(agent_distribution['house'][0])]
                        count += agent_distribution['house'][0]
                    elif agent_subtype == 1:
                        list_subtype_agent = [Agent(7,7,original_agents_attributes[count+i], .15, .95, .05)
                                              for i in range(agent_distribution['house'][1])]
                        count += agent_distribution['house'][1]
                    elif agent_subtype == 2:
                        list_subtype_agent = [Agent(8,8,original_agents_attributes[count+i], .1, .1, .95)
